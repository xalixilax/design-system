...
<AnimatePresence>
  {items.map((item) => (
    <motion.li key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} layout>
      {item.text}
    </motion.li>
  ))}
</AnimatePresence>
...
return <div ref={parent}>{isOpen && <div>Content</div>}</div>;
```
...
```tsx
<motion.div drag="x" dragConstraints={{ left: -width, right: 0 }}>
  {images.map((img) => (
    <img src={img.url} />
  ))}
</motion.div>
...
```tsx
<motion.div drag dragConstraints={constraints} onDragEnd={handleDragEnd}>
  Draggable item
</motion.div>
```
