...
function canResizeSegmentPair(
  ...
  const neighbor = sections.find((candidate) => candidate.id === neighborSectionId);

  if (!section || !neighbor) {
    return false;
  }

  const participants = buildSegmentParticipants(sections, section, neighbor, direction);
    ...
    if (eastBoundary <= bounds.columns) {
      const neighbors = sections.filter(
        (candidate) =>
          ...
          rangesOverlap(section.y, section.y + section.h, candidate.y, candidate.y + candidate.h),
      );

      for (const neighbor of neighbors) {
        const canMove = section.w > (section.minW ?? 2) || neighbor.w > (neighbor.minW ?? 2);
        if (!canMove) {
          continue;
        }

        const segmentStart = Math.max(section.y, neighbor.y);
        const segmentEnd = Math.min(section.y + section.h, neighbor.y + neighbor.h);
        ...
        const length = segmentLengthRows * rowSize + (segmentLengthRows - 1) * gap;
        separators.push({
          ...
          length,
          localResizable: canResizeSegmentPair(sections, bounds, section.id, neighbor.id, "e"),
        });
      }
    }
    ...
    if (southBoundary <= bounds.rows) {
      const neighbors = sections.filter(
        (candidate) =>
          ...
          rangesOverlap(section.x, section.x + section.w, candidate.x, candidate.x + candidate.w),
      );

      for (const neighbor of neighbors) {
        const canMove = section.h > (section.minH ?? 2) || neighbor.h > (neighbor.minH ?? 2);
        if (!canMove) {
          continue;
        }

        const segmentStart = Math.max(section.x, neighbor.x);
        const segmentEnd = Math.min(section.x + section.w, neighbor.x + neighbor.w);
        ...
        const length = segmentLengthColumns * columnSize + (segmentLengthColumns - 1) * gap;
        separators.push({
          ...
          offsetY: offsetY + gridPaddingPx,
          length,
          localResizable: canResizeSegmentPair(sections, bounds, section.id, neighbor.id, "s"),
        });
      }
    }
  }
...
export function buildBoundaryGuides(
  ...
  for (const [groupKey, group] of grouped.entries()) {
    const sortedRanges = [...group.ranges].sort((left, right) => left.start - right.start);
