...
export function buildSplitPreview(
  ...
  const useVertical = section.w >= section.h ? canSplitVertically : !canSplitHorizontally;
...
export function getSplitPlacementFromPreview(preview: Extract<NewSectionPreview, { mode: "split" }>): SplitPlacement {
  if (preview.orientation === "vertical") {
    return {
      orientation: "vertical",
