...
function buildSegmentParticipants(
...
export function useDashboardLayout({ bounds, initialSections }: UseDashboardLayoutOptions) {
  ...
  const splitSectionWithNew = useCallback(
    (hoveredSectionId: string, cursorX: number, cursorY: number) => {
      setSections((previous) => {
        const hoveredSection = previous.find((section) => section.id === hoveredSectionId);
        if (!hoveredSection) {
          return previous;
        ...
        const splitVerticalPreferred = hoveredSection.w >= hoveredSection.h;
        ...
        const useVertical = splitVerticalPreferred ? canSplitVertically : !canSplitHorizontally;
        const nextIndex = previous.length + 1;
        ...
        if (useVertical) {
          ...
          const dropOnLeft = cursorX < hoveredSection.x + hoveredSection.w / 2;
          ...
          const dropOnTop = cursorY < hoveredSection.y + hoveredSection.h / 2;
